...
static err_t UDP_Create() {
	...
	err = netconn_bind(conn, IP_ADDR_ANY, bsp.ip4_current.udp_port);
...
static err_t UDP_Send(char *data, uint32_t bytes_count, addon_t addon) {
	...
	if (NULL != buf) {
		if (ADD_TAIL == addon) {
			payload = netbuf_alloc(buf, bytes_count + 2);
		} else {
		...
		if (NULL != payload) {
			...
			if (ADD_TAIL == addon)
				memcpy(payload + bytes_count, end, 2);
...
static err_t UDP_SendV2(char *data, uint32_t bytes_count, addon_t addon) {
	...
	if (NULL != buf) {
		...
		case (ADD_HEAD_AND_TAIL):
			payload = netbuf_alloc(buf, bytes_count + start.len + 2);
			break;
		default:
			payload = netbuf_alloc(buf, bytes_count);
			break;
		...
		if (NULL != payload) {
			...
			case (ADD_HEAD_AND_TAIL): {
				memcpy(payload, start.data, start.len);
				memcpy(payload + start.len, data, bytes_count);
				memcpy(payload + start.len + bytes_count, end, 2);
...
static err_t UDP_SendREAL() {
	...
	tx = UTIL_FloatArrayToREAL(&measurements[0], sample_count);
...
static err_t UDP_SendASCII() {
	...
	tx = UTIL_FloatArrayToASCII(&measurements[0], sample_count);
	size = strlen(tx);
	loop = size / UDP_PACKAGE_SIZE;
	rest = size % UDP_PACKAGE_SIZE;
	index = 0;
...
void StartUDPTask(void const *argument) {
	...
	for (;;) {
		if (pdTRUE == xQueueReceive(QueueUDPHandle, &state, 5U)) {
		...
		case UDP_STATE_ERROR:
			LED_osQueue(RED);
			break;
		case UDP_STATE_IDLE: /* Do nothing */
			;
			break;
		default: /* Do nothing */
			break;
